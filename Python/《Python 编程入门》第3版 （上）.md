...
>**提示**
 键盘读取字符串时，另一种常见的实用技巧是，使用函数 `strip()`将开头和末尾的空白字符删除  
...
>**后期补充**		
 linux命令行下的方法：     
 保存 area.py的文件夹下启动Python命令行，输入`python3`； 			
 将其导入 ` from area import * `		
 用示例： >>> area(10)			
 果使用 import area 导入，则使用示例为： >>> area.area(10)      
 意：1. 要在同一文件夹下。   2. 作为模块导入时不用加上后缀名 `.py`。   
...
>**Unicode 的崛起**    
>Unicode 提供了一个大的编码集。    
 于方便，Unicode的前256个字母为 ASCII码，     
 此如果你只处理英文字符，几乎不用考虑Unicode的细节。   
 关Unicode可上其[主页](http//:www.unicode.org)了解     
...
>函数`split`总是返回一个字符串列表；		
 ython中列表总是分别以 ` [  ` 和 ` ] ` 打头和结尾，		
 用逗号 ` , `分隔元素。		
	表和元组很像，主要差别在于列表是可以修改的，而元组是常量。		
