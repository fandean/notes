...
**可重入锁的底层实现原理：**
 ...
 每次成功的申请锁以后，要记录下到底是谁申请的， 还要用一个计数器记录重入的次数， 下一次持有锁的家伙再次申请锁只是给计数器加一而已。
...
>
> [Java 中15种锁的介绍：公平锁，可重入锁，独享锁，互斥锁，乐观锁，分段锁，自旋锁等等 - Java开发社区 | CTOLib码库](https://www.ctolib.com/topics-137596.html)
>
...
- 利用**数据库**的**行锁**实现分布式锁（参考Quartz定时执行框架）。如图：
  ...
  而没有获得锁的线程会被**阻塞**，等到行锁被释放，它可以自动去获取，代码中不用循环重试。（**这里一个重要思想：避免循环重试的方案之一是线程阻塞**，（了解多线程后这里还需要补充））
